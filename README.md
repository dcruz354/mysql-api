...
After you've cloned the project, you'll need to have a database created in mysql called orderdb.
...
```javascript
// order.queries.js
exports.CREATE_ORDERDETAILS_TABLE = `CREATE TABLE IF NOT EXISTS tasks(...)`;

// order.controller.js
const queries = require('path/to/queries');
con.query(queries.CREATE_ORDERDETAILS_TABLE, params, callback);
```

## Routes

Routes help direct what an `endpoint` should do. In RESful services, `endpoints` are the full url of a given API at a specific address in that API. For example: `http://localhost:3000/api/orderdetails` is a RESTful `endpoint`. Pointing my browser at this address (assuming the server is live) will give me results if a route exists and has a [controller method](#controllers) attached.
...
```javascript
exports.addOrder = (req, res) => { ... } // function signature: (req, res) => {}
```
...
con.connect(function(err) {
  ...
  // orderdb becomes the created database upon connection if one doesn't exist
  con.query(`CREATE DATABASE orderdb`, function(err, result) {
    if (err) throw err;
